...
describe("UserConsumer Integration in Memory", () => {
    afterEach(() => {
        sinon.restore();
    });
    ...
    it("should run a full E2E flow with multiple user consumers and steps", async () => {
        ...
        const consumer = new UserConsumer({ transport: TransportType.MEMORY });
    ...
    it("should run a full E2E flow with user consumer and throw exception", async () => {
        ...
        expect(
            await checkExceptionCalledAsync(spyBegin, Exception, "Bad user id")
        ).to.be.true;
